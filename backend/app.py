...
from resources.participants import LoadParticipants,Assign,Participants,Participant,GenerateReport,ParticipantHistory
...
api.add_resource(JurorByParticipant,"/api/jurors/participant")
...
api.add_resource(ParticipantHistory,"/api/participant/history")
api.add_resource(GenerateReport,"/api/report")
...
def admin_background_thread():
    while True:
        socketio.sleep(2)
        collection = db.participants
        participants = json.loads(json_util.dumps(list(collection.find({}))))
